...
def insert_hf_models(engine):
    file_path = "json/models/HF_model_top_and_bottom_10.json"
...
def insert_github_issues(engine):
    files = [file for file in pathlib.Path("json/sample").glob("*issues.json")]
    with Session(engine) as session:
...
def insert_github_prs(engine):
    files = [file for file in pathlib.Path("json/sample").glob("*prs.json")]
    with Session(engine) as session:
        file_names = []
        for file in tqdm(files):
...
def get_reuse_repos(reuse_repos_to_keep):
    reuse_repos = {}
    ...
    for file in files:
        ...
        if owner_repo not in reuse_repos_to_keep:
            continue
...
def insert_reuse_repos(engine, reuse_repos_to_keep: list):
    reuse_repos = get_reuse_repos(reuse_repos_to_keep)
    reuse_repo_list = []
    with Session(engine) as session:
        ...
        for link_file in files:
            ...
            with open(link_file, "r") as json_file:
                ...
                for model_context_id in tqdm(links.keys()):
                    if model_context_id not in ids:
                        continue
...
def insert_pt_models(engine, pt_model_file = "json/models/PT_model_top_and_bottom_10.json"):
    with Session(engine) as session:
...
if __name__ == "__main__":
    engine = create_engine("sqlite:///PeaTMOSS_SAMPLE.db")

    BASE.metadata.drop_all(engine)
    BASE.metadata.create_all(engine)
    ids = [id for id in etl.fromjson("json/models/pt_and_hf_ids.json").values("context_id")]
    temp = etl.fromjson("json/models/ptm_to_gh_converted.json").values("usage_repository")
    reuse_repos = []
    for repo_list in temp:
        reuse_repos.extend(repo_list)
    reuse_repos = list(set(reuse_repos))
    # hf_to_gh = etl.fromjson('json/models/all_hf_slim.json')
    # pt_to_gh = etl.fromjson('json/models/all_pt_slim.json')
    # hf_to_gh = etl.cut(hf_to_gh, "usage_repository").convert("usage_repository", lambda v: v[0:10])
    # pt_to_gh = etl.cut(pt_to_gh, "usage_repository").convert("usage_repository", lambda v: v[0:10])
    # ptm_to_gh = etl.cat(hf_to_gh, pt_to_gh)
    # etl.tojson(ptm_to_gh, "json/models/ptm_to_gh.json", indent=4)
    # ptm_to_gh = etl.fromjson("json/models/ptm_to_gh.json")
    # ptm_to_gh = etl.convert(ptm_to_gh, "usage_repository", lambda v: ["_".join(val.rsplit("/")[1:3]) for val in v])
    # ptm_to_gh = etl.tojson(ptm_to_gh, "json/models/ptm_to_gh_converted.json", indent=4)
    # insert_github_prs(engine)
    # insert_github_issues(engine)
    insert_reuse_repos(engine, reuse_repos)
    insert_github_issues(engine)
    insert_github_prs(engine)
    insert_model_hubs(engine)
    insert_pt_models(engine)
    insert_hf_models(engine)
    link_ptm_gh(engine, ids)
