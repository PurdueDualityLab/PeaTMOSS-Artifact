...
class MNode:
    ...
    def parse_func_defs(self):
        ...
        # func: function
        def_records = []
        self._parse_func_defs(ast.body, def_records, scope="mod")
        return def_records
    ...
    def parse_import_stmts_extended(self):
        """
        Return a dictionary data structure to map the imported name, from which
        module and its aliases.
        """
        ...
        for stmt in import_stmts:
            ...
            if isinstance(stmt, ast.ImportFrom):
                ...
                if m_name is None and stmt.level== 2:
                    m_name = '..'
                if m_name is None and stmt.level== 3:
                    m_name = '...'
    ...
    def parse_func_defs_extended(self):
        ...
        # func: function
        def_records = []
        self._parse_func_defs_extended(self.ast.body, def_records, scope = "mod")
        return def_records


    def _parse_func_defs_extended(self, ast_node_lst, def_records, scope = "mod"):
        """
        Parse the function and class definitions in this module
        Args:
            ast_node_lst: a list of statements to be visited.
        ...
        for node in ast_node_lst:
            ...
            if isinstance(node, ast.FunctionDef):
                def_info = {"scope":scope, "name": node.name, "arg": [], "kws": [],
                        "lineno":node.lineno, "col_offset":node.col_offset, "end_linno": node.end_lineno}
                def_records.append(def_info)
                self._parse_func_defs_extended(node.body, def_records, scope=node.name)

            elif isinstance(node, ast.ClassDef):
                #visit class body
                # to consider class records
                # args_func.append([val for val in node.args])
                def_info = {"scope":scope, "name": node.name, "arg": [], "kws": [],
                        "lineno":node.lineno, "col_offset":node.col_offset, "end_linno": node.end_lineno}
                def_records.append(def_info)
                self._parse_func_defs_extended(node.body, def_records, scope=node.name)
